 ...
 * @access  Private
 */
router.get('/profile', requireAuth, requireOwnProfile, async (req, res) => {
  try {
 ...
 * @access  Private
 */
router.put('/profile', requireAuth, requireOwnProfile, validateProfileUpdate, async (req, res) => {
  try {
    const updateData = req.body;
    const user = await AuthService.updateUserProfile(req.user.user_id, updateData);
...
router.get('/admin/users', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    logger.error('Get all users endpoint error:', error);
...
router.put('/admin/users/:userId/role', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    logger.info(`User role updated by admin ${req.user.email}: ${user.email} -> ${role}`);
